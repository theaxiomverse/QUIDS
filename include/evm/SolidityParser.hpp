...
// Define DataType and TypeInfo to resolve the undefined identifier error
...
struct TypeInfo {
    ...
    std::shared_ptr<TypeInfo> mapping_key_type;   // For mappings
    std::shared_ptr<TypeInfo> mapping_value_type; // For mappings

    TypeInfo(const TypeInfo&) = default;
    TypeInfo& operator=(const TypeInfo&) = default;
};

// Added global Token definition to resolve 'Token' undeclared error
struct Token {
    enum class Type {
        KEYWORD,
        IDENTIFIER,
